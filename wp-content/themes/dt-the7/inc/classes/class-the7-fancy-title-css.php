...
class The7_Fancy_Title_CSS {
	...
	public static function add_less_vars_from_post( $post_id, Presscore_Lib_LessVars_Manager $less_vars ) {
		...
		$less_vars->add_keyword( 'fancy-header-text-transform', ( get_post_meta( $post_id, '_dt_fancy_header_uppercase', true ) ? 'uppercase' : 'none' ) );
		...
		$less_vars->add_keyword( 'fancy-header-subtitle-text-transform', ( get_post_meta( $post_id, '_dt_fancy_header_subtitle_uppercase', true ) ? 'uppercase' : 'none' ) );
